...
const TargetManagement = () => {
    ...
    const fetchTargets = useCallback(async () => {
        try {
            ...
            // For both admin and user - fetch all targets based on filters
            if (filterYear) params.append("year", filterYear);
            if (filterMonth) params.append("month", filterMonth);
    ...
    }, [userRole, filterYear, filterMonth, filterUser]);
    ...
    const applyFilters = () => {
        let data = [...users];
        // For user role - show all users (so they can see everyone's data)
        if (userRole !== "admin" && filterUser) {
            data = data.filter(u => u.id === Number(filterUser));
        }
        return data;
    ...
    // ================= MODAL HANDLERS =================
    const openModal = (user) => {
        // Only admin can open target modal
        if (userRole !== "admin") {
            toast.error("You don't have permission to edit targets");
            return;
        }
        ...
        if (user) {
            setSelectedUser(user);
            setSelectedUserId(user.id);
            const existingTarget = targets.find(t => t.user_id === user.id && t.year === parseInt(year) && t.month === parseInt(month));
            setMonthlyTarget(existingTarget ? existingTarget.target : "");
        } else {
            setSelectedUser(null);
            setSelectedUserId("");
            setMonthlyTarget("");
    ...
    const handleSetTarget = async (e) => {
        e.preventDefault();
        if (userRole !== "admin") {
            toast.error("You don't have permission to set targets");
            return;
        }
    ...
    // Get current month target for logged-in user's stats
    const getCurrentMonthTarget = () => {
        return targets.find(t => t.user_id === userId && t.year === currentYear && t.month === currentMonth);
    };

    // Get display users - for both admin and regular user, show all users
    const displayUsers = filterUser ? users.filter(u => u.id === Number(filterUser)) : users;
    const paginatedUsers = displayUsers.slice(indexOfFirstUser, indexOfLastUser);
    
    // For user role - they can see all users but no actions
    ...
    return (
        <div className="target-management-container">
            ...
            <div className="container mt-4">
                ...
                <div className="admin-header">
                    <div className="header-left">
                        <h3>{userRole === "admin" ? "Team Performance Overview" : "All Team Members Performance"}</h3>
                        ...
                        {(filterYear || filterMonth || filterUser) && (
                            <div className="filter-badge-admin">
                                🔍 Filtered results
                                <button className="clear-filters-btn-admin" onClick={clearAllFilters}>Clear Filters</button>
                            </div>
                        )}
                    </div>
                    {userRole === "admin" && (
                ...
                {/* Stats Cards - Only for logged-in user (their own stats) */}
                {userRole !== "admin" && (
                    <div className="stats-grid">
                        {(() => {
                            ...
                            return (
                                <>
                                    <div className="stat-card-primary">
                                        <div className="stat-icon">🎯</div>
                                        <div className="stat-info">
                                            <span>My Current Month Target</span>
                                            <strong>{currentTarget ? currentTarget.target.toLocaleString() : 0}</strong>
                                        </div>
                                    </div>
                                    <div className="stat-card-success">
                                        <div className="stat-icon">✅</div>
                                        <div className="stat-info">
                                            <span>My Current Month Achieved</span>
                                            <strong>{currentTarget ? currentTarget.achieved.toLocaleString() : 0}</strong>
                                        </div>
                                    </div>
                                    <div className="stat-card-warning">
                                        <div className="stat-icon">📊</div>
                                        <div className="stat-info">
                                            <span>My Current Month Progress</span>
                                            <strong>{currentTarget ? `${currentTarget.progress}%` : "0%"}</strong>
                                        </div>
                                    </div>
                                </>
                            );
                ...
                <div className="filter-container">
                    <div className="filter-group">
                        <label>Year</label>
                        <select className="custom-select" value={filterYear} onChange={(e) => setFilterYear(e.target.value)}>
                            <option value="">All Years</option>
                ...
                {/* Modern Table - Shows ALL users data for both admin and regular user */}
                <div className="table-container">
                    <table className="modern-table">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            {paginatedUsers.length > 0 ? paginatedUsers.map(user => {
                                                ...
                                                {idx === 0 && (
                                                    <td rowSpan={userTargetsList.length} className="user-cell">
                                                        ...
                                                        <span className="user-name">
                                                            {user.name}
                                                            {isLoggedInUser && userRole !== "admin" && <span className="current-tag-admin" style={{marginLeft: "8px"}}> (Me)</span>}
                                                        </span>
                                                    </td>
                                                )}
                                                <td>
                                ...
                                <tr>
                                    <td colSpan={showActions ? "7" : "6"} className="empty-state">
                ...
                {/* Pagination */}
                {displayUsers.length > usersPerPage && (
                    <div className="pagination-modern">
                        <button onClick={() => setCurrentPage(p => Math.max(1, p-1))} disabled={currentPage === 1}>← Prev</button>
                        {Array.from({ length: totalPages }, (_, i) => (
                            <button key={i} className={currentPage === i+1 ? 'active' : ''} onClick={() => setCurrentPage(i + 1)}>{i + 1}</button>
                ...
                {/* Chart Card - Only for logged-in user (their own chart) */}
                {userRole !== "admin" && (
                    <div className="chart-card" style={{ marginTop: "30px" }}>
                            ...
                            <span className="chart-badge">
                                {(() => {
                                    ...
                                    return currentTarget ? 
                                        `${currentTarget.progress}% Complete (Current Month)` : 
                                        "No Data";
                                })()}
                            </span>
                        </div>
                        <div className="chart-body">
                            {(() => {
                                const currentTarget = getCurrentMonthTarget();
                                const chartDataLocal = currentTarget && (Number(currentTarget.achieved) > 0 || Number(currentTarget.remaining) > 0) ? {
                                    labels: ["Achieved", "Remaining"],
                                    datasets: [{
                                        data: [Number(currentTarget.achieved), Number(currentTarget.remaining)],
                                        backgroundColor: ["#28a745", "#dc3545"],
                                        borderWidth: 1,
                                    }],
                                } : null;
                                    ...
                                    <>
                                        ...
                                        <div className="chart-stats">
                                            <div className="stat-item">
                                                ...
                                                <span className="stat-value">{currentTarget.target.toLocaleString()}</span>
                                            </div>
                                            <div className="stat-item success">
                                                <span className="stat-label">Achieved</span>
                                                <span className="stat-value">{currentTarget.achieved.toLocaleString()}</span>
                                            </div>
                                            <div className="stat-item danger">
                                                <span className="stat-label">Remaining</span>
                                                <span className="stat-value">{currentTarget.remaining.toLocaleString()}</span>
                                            </div>
                                        </div>
                                    </>
                                ) : (
                        ...
                        <form onSubmit={handleSetTarget}>
                            <div className="modal-body">
                                {!selectedUser && (
                                    <div className="form-group">
                                        <label>Select User</label>
                                        <select className="custom-select" value={selectedUserId} onChange={(e) => setSelectedUserId(e.target.value)} required>
                                            <option value="">Choose a team member</option>
                                            {users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                                        </select>
                                    </div>
                                ...
                                <div className="form-row">
                                    ...
                                    <div className="form-group">
                                        <label>Month</label>
                                        <select className="custom-select" value={month} onChange={(e) => setMonth(e.target.value)} required>
                                            {[...Array(12)].map((_, i) => <option key={i} value={i+1}>{getMonthName(i+1)}</option>)}
                                        </select>
                                    </div>
                                </div>
